...
use crate::{
  ApplyContext, BoxedGeneratorBuilder, BoxedParserBuilder, CompilationHooks, CompilationId,
  CompilerHooks, CompilerOptions, ConcatenatedModuleHooks, ContextModuleFactoryHooks, ModuleType,
...
pub struct PluginDriver {
  ...
  #[debug(skip)]
  pub registered_parser_builder: FxDashMap<ModuleType, BoxedParserBuilder>,
  #[debug(skip)]
  pub registered_generator_builder: FxDashMap<ModuleType, BoxedGeneratorBuilder>,
...
impl PluginDriver {
  pub fn new(
    ...
    let mut registered_parser_builder = FxDashMap::default();
    let mut registered_generator_builder = FxDashMap::default();
    let mut apply_context = ApplyContext {
      registered_parser_builder: &mut registered_parser_builder,
      registered_generator_builder: &mut registered_generator_builder,
